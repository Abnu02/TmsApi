...
public class InMemoryTranscriptStatusStore : ITranscriptStatusStore
{
    ...
    public Task MarkProcessingAsync(string reportId, CancellationToken ct) =>
        Transition(reportId, current => current with { State = TranscriptState.Processing, StartedAt = DateTimeOffset.UtcNow }, allowedFrom: TranscriptState.Queued);

    public Task MarkReadyAsync(string reportId, string downloadUrl, CancellationToken ct) =>
        Transition(reportId, current => current with { State = TranscriptState.Ready, CompletedAt = DateTimeOffset.UtcNow, DownloadUrl = downloadUrl }, allowedFrom: TranscriptState.Processing);

    public Task MarkFailedAsync(string reportId, string error, CancellationToken ct) =>
        Transition(reportId, current => current with { State = TranscriptState.Failed, CompletedAt = DateTimeOffset.UtcNow, ErrorMessage = error }, allowedFrom: TranscriptState.Processing);
    ...
    private Task Transition(string reportId, Func<TranscriptStatus, TranscriptStatus> change, TranscriptState allowedFrom)
    {
        ...
        if (current.State != allowedFrom)
            throw new InvalidOperationException($"Cannot move {reportId} from {current.State} via this transition (expected {allowedFrom}).");
